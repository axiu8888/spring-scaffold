...
public class QuartzUtils {
  ...
  public static void checkWorker(QrtzJobTask task) {
    ...
    if (workerType == null) {
      throw new LogicException("WorkerType错误!");
    }

    if (StringUtils.isBlank(task.getWorker())) {
      throw new LogicException("worker不能为空");
    }
    ...
    if (workerType != WorkerType.SPRING_BEAN_NAME) {
      try {
        ...
        throw new LogicException("请指定正确的worker");
      }
    } else {
      ...
      if (!(ctx.containsBean(worker)) || !(ctx.getBean(task.getWorker()) instanceof JobWorker)) {
        throw new LogicException("未发现匹配的JobWorker实例!");
      }
    }
  }
  ...
  public static TriggerBuilder<Trigger> trigger(QrtzJobTask task) {
    ...
    if (triggerType == null) {
      throw new LogicException("触发器类型错误");
    }
